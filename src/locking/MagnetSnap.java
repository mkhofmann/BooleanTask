...
public class MagnetSnap extends LockingMechanism {
	...
	private Polyhedron prism(){
		ArrayList<Coordinate> points = new ArrayList<Coordinate>(9);
		...
		faces.add(new Face(6,7,2,1));//back
	...
	protected void makeLock() {
		Union k = this.SolidPrism();
		Difference diff = new Difference(new ArrayList<Node>(3));
		diff.addChild(new Cylinder(radius,height));
		diff.addChild(k);
		diff.addChild(new Translate(new Rotate(super.magnet(),-90,0,-45),-indent-LockingMechanism.MAGH,-indent-LockingMechanism.MAGH,this.height/2));//TODO: Magnet hole not clean
		super.lock=new SCADModel(diff,new SizingBlock(new Coordinate(radius*2,radius*2,height), new Coordinate(-radius, -radius, 0)));
...
//		super.lock=new SCADModel(diff, new SizingBlock(radius, height));
	...
	public static void main(String[] arg){
		...
		SCADWriter.writeSCAD(snap.encodeMechanism(), "magnetSnap");
	}
}
